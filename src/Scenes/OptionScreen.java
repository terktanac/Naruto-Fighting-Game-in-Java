...
public class OptionScreen extends myScene {
	...
	private int OldChoice_1;
	private int NewChoice_1;
	private int OldChoice_2;
	private int NewChoice_2;
	...
	public void keyHandling() {
		setOnKeyPressed(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				...
				MediaPlayer choose = new MediaPlayer(
						new Media(ClassLoader.getSystemResource("accept5.wav").toString()));
				...
				if (setState1 && (key == keyP1.get(OldChoice_1) || !keyP1.contains(key)) && !keyP2.contains(key)) {
					...
					timeline1.play();
						...
						&& (key == keyP2.get(OldChoice_2) || !keyP2.contains(key))) {
	...
	public class ListOption extends HBox {
		...
		ListOption(String text, int player) {
			this.setBackground(new Background(new BackgroundImage(
					new Image(ClassLoader.getSystemResource("icon/frame.png").toString(), 230, 60, false, true), null,
					null, null, null)));
			this.player = player;
			this.setSpacing(10);
